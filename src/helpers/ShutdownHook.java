...
public class ShutdownHook {
	 ...
	 * @param rq The shared queue object where completed jobs -along with their
	 * results- are being put
	 * @param th The structure containing all t objects to join them when
 termination signal is caught.
	 */
	public ShutdownHook(BlockingQueue<NmapJob> jq, BlockingQueue<NmapJob> rq, ArrayList<Thread> th) {
		this.jobQueue = jq;
		this.resultQueue = rq;
		this.threadHotel = th;
	}
				...
				for (Thread t : threadHotel) {
					if (t.isAlive()) {
						...
						try {
							...
							System.err.println("Thread was interrupted while blocked!");
						}
					}
				}
